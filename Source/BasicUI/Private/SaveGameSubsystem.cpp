...
void USaveGameSubsystem::RebindAxisMap(FName AxisName, float Scale, FInputChord NewKey)
{
	...
	if (ensure(SaveSettings))
	{
		...
		// Save settings
		UGameplayStatics::SaveGameToSlot(SaveSettings, SaveName, 0);
...
void USaveGameSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	SaveSettings = GetGameSave<USettingsSaveGame>(SaveName);
	//ensure(SaveSettings);
}
...
USaveGame* USaveGameSubsystem::GetGameSave(FString SlotName, UClass* SaveType)
{
	...
	if (bSaveExists)
	{
		...
		UE_LOG(LogTemp, Warning, TEXT("Load save file!"));
	}
	else
	{
		return UGameplayStatics::CreateSaveGameObject(SaveType);
		UE_LOG(LogTemp, Warning, TEXT("Create save file!"));
	}
}
